...
// Timestamp | Email | Affiliate | Plan | Amount | Currency |
// Client Reference ID | Session ID | Purchase Source | Referral Method | Premium Activated
//
// EXAMPLE ROW WRITTEN:
// ["2026-04-08T02:00:00.000Z","john@example.com","WILLSIZE","yearly","99.00","USD",
...
export async function appendTransactionToSheet({
  ...
  if (!sheetId) {
    console.warn("WARN [RvWay] GOOGLE_SHEETS_ID not set - skipping Sheets log");
    return;
  }
  ...
  try {
    ...
    console.log("OK [RvWay] Sheets row appended successfully for session_id=" + sessionId);
  } catch (err) {
    console.error("ERR [RvWay] Google Sheets append failed:", err.message);
    // Do not rethrow - Sheets failure must not affect premium activation
  }
                }
